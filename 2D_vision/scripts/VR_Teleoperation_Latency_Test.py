	...
	def run(self):
		...
		while self.running:
			...
			if data != None: # if NEW data has been received since last ReadReceivedData function call
				cur_time = time.time()
				latencies.append(cur_time-last_time)
				last_time = cur_time
				...
				# Move the gripper.
				fa.goto_gripper(goal_width, block=False, speed=0.15, force = 60) # , force = 10)
				
				if len(latencies) %100 == 0:
					print('Latency len', len(latencies))

				if len(latencies) > 1100:
					print("breaking")
					break
		
		print("Saving Latency")
		with open("/home/franka/frankapy/vision_module/2D_vision/scripts/Latency Data/Run" +str(int(np.random.uniform(0, 10000))) + ".pkl", 'wb') as pickle_file:
			pkl.dump(latencies[-1000:], pickle_file)
...
if __name__ == "__main__":
	...
	TeleopController = Teleoperation(franka_IP = "172.26.18.135", Oculus_IP = "172.26.33.175")
	TeleopController.start()
	
	#Object demo, generates a block every 10 seconds.
	object_dictonary = {}
	for i in range(4):
		object_dictonary[i] = Object(position=[0.6, 0, i/10])
