...
def vision_loop(realsense_intrinsics_ee, realsense_to_ee_transform, realsense_intrinsics_static, realsense_to_static_transform, detected_objects, object_queue):
	...
	while True:
		...
		# both = set(detections1) & set(detections2)
		...
		for d in all_detections:
			...
			# apritag only detected by static camera
		...
		# 		# add tag to the dictionary of detected objects
		# 		tagFamily = d.tag_family.decode("utf-8")
		# 		detected_objects.update({obj_id : DetectObject(realsense_intrinsics_ee, realsense_to_ee_transform, realsense_intrinsics_static, realsense_to_static_transform, object_id=obj_id, object_class=tagFamily)})
